...
species cleaner skills:[pedestrian]{
	...
	reflex reset_total_duration when: every (7#day){
...
//experiment 'Run 5 simulations' type: batch repeat: 4 keep_seed: true until: ( cycle > 10080 ) {
//	   
//	   
//    	
//	reflex abc{
...
//			ask simulations{
//				ask cleaner{
//		       		save [world.cycle, world.weekly_complain_log] to: "result" + adder + ".csv" format: "csv";
//		       		adder <- adder+1;
//		       	 
//		       	}
//	    	}
...
experiment pedestrian_navigation type: gui {
    ...
    output {
		...
		display "Complaints" type: 2d{
			chart "Weekly Cumulative Complaints" type: series x_label:"Simulation Runtime (seconds)" y_label:"Count"  {
		...
		display "Overflow" type: 2d{
			chart "Weekly Cumulative Overflow Bin Duration" type: series x_label:"Simulation Runtime (seconds)" y_label:"Duration (seconds)"  {
		...
		display "Trigger Cleaning" type: 2d{
			chart "Total Daily Cleaning Cycles Triggered" type: series x_label:"Simulation Runtime (seconds)" y_label:"Count"  {
		...
		display "Distance Travelled by Cleaner" type: 2d{
			chart "Daily Cumulative Distance Travelled by Cleaner" type: series x_label:"Simulation Runtime (seconds)" y_label:"Distance (metres)"  {
				
                data "Distance Travelled" value: (cleaner collect ((each.total_time_travelled* each.speed/3600 *1000) with_precision 2)) color: #indigo marker_size: (0.05) // size keyword instead of size in values
				marker_shape: marker_circle style: bar;
        ...
        display vidacity_lvl_1 type: opengl background: #lightgrey{
        	...
        	// Legend 
             overlay position: { 20, 470 } size: { 1200 #px, 250 #px } background: #black transparency: 0.3 border: #black rounded: true
            {
                float y <- 30#px;
                	...
                	draw ("TOTAL COMPLAINTS PER WEEK: " + weekly_complain_log)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                	...
                	draw ("ACCUMULATED TIME OF BINS OVERFLOWED (MINS/WEEK): " + (weekly_duration_overflow_log/60) with_precision 2)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                    ...
                    ask cleaner{                	
                    	draw ("CLEANER DISTANCE TRAVELLED (METRES/DAY): " + (self.total_time_travelled/3600*self.speed*1000) with_precision 2)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                    }
