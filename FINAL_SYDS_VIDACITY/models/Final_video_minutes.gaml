...
//	reflex die when: cycle = 10080{
//		write"Number of complaints: " + total_complaints;
//		ask cleaner{
//			write "Total distance: " + (self.total_time_travelled/60*self.speed*1000) with_precision 2;
//		}
//		
//		do die;
//	}
...
////////////////////////////////////////// LEVEL 1 //////////////////////////////////////////
	...
	int lvl1_store_0_no_tenants <- 7 parameter:true category:"Level 1";

	int lvl1_store_1_no_tenants <- 7 parameter:true category:"Level 1";

	int lvl1_store_2_no_tenants <- 7 parameter:true category:"Level 1";
	
	int lvl1_store_3_no_tenants <- 4 parameter:true category:"Level 1";

	int lvl1_store_4_no_tenants <- 6 parameter:true category:"Level 1";

	int lvl1_store_5_no_tenants <- 5 parameter:true category:"Level 1";

	int lvl1_store_6_no_tenants <- 7 parameter:true category:"Level 1";

	int lvl1_store_7_no_tenants <- 3 parameter:true category:"Level 1";
...
////////////////////////////////////////// LEVEL 2 //////////////////////////////////////////
	...
	int lvl2_store_10_no_tenants <- 7 parameter:true category:"Level 2"; 
	
	int lvl2_store_11_no_tenants <- 3 parameter:true category:"Level 2";
	
	int lvl2_store_12_no_tenants <- 7 parameter:true category:"Level 2";
	...
	int lvl2_store_15_no_tenants <- 6 parameter:true category:"Level 2";

	int lvl2_store_16_no_tenants <- 7 parameter:true category:"Level 2";
	
	int lvl2_store_17_no_tenants <- 8 parameter:true category:"Level 2";
	
	int lvl2_store_18_no_tenants <- 3 parameter:true category:"Level 2";
...
////////////////////////////////////////// LEVEL 3 //////////////////////////////////////////
	...
	int lvl3_store_24_no_tenants <- 3 parameter:true category:"Level 3";
	...
	int lvl3_store_27_no_tenants <- 2 parameter:true category:"Level 3";
...
////////////////////////////////////////// LEVEL 4//////////////////////////////////////////
	...
	int lvl2_store_29_no_tenants <- 7 parameter:true category:"Level 4";

	int lvl2_store_30_no_tenants <- 3 parameter:true category:"Level 4"; 
...
species cleaner skills:[pedestrian]{
	...
	reflex reset_total_duration when: every (1#day){
		self.total_time_travelled <- 0.0;
	}
...
//experiment 'Run 5 simulations' type: batch repeat: 5 keep_seed: true until: ( time > 500 ) {
//    int cpt <- 0;
//
//    reflex save_people {
//    	ask simulations{
//    		save data: weekly_complain_log to: "../result.csv" format: "csv";
//    	}
//    }
//}
...
experiment pedestrian_navigation type: gui {
    ...
    output {
		...
		display "Complaints" type: 2d{
			chart "Weekly Cumulative Complaints" type: series x_label:"Simulation Runtime (minutes)" y_label:"Count"  {
                data "Complaint Count" value:weekly_complain_log color: #burlywood marker_size: (0.05)  // size keyword instead of size in values
				marker_shape: marker_circle style:bar  ;
			}
		}        
       
		display "Overflow" type: 2d{
			chart "Weekly Cumulative Overflow Bin Duration" type: series x_label:"Simulation Runtime (minutes)" y_label:"Duration (minutes)"  {
                data "Overflow Duration" value:weekly_duration_overflow_log color: #slategrey marker_size: (0.05)  // size keyword instead of size in values
				marker_shape: marker_circle style: bar ;
			}
		}  
		
		display "Trigger Cleaning" type: 2d{
			chart "Total Daily Cleaning Cycles Triggered" type: series x_label:"Simulation Runtime (minutes)" y_label:"Count"  {
                data "Cleaning Cycle Count" value:daily_cleaner_trigger_count color: #teal marker_size: (0.05)  // size keyword instead of size in values
				marker_shape: marker_circle style: bar ;
			}
		} 

		display "Distance Travelled by Cleaner" type: 2d{
			chart "Daily Cumulative Distance Travelled by Cleaner" type: series x_label:"Simulation Runtime (minutes)" y_label:"Distance (metres)"  {
				
                data "Distance Travelled" value: (cleaner collect ((each.total_time_travelled* each.speed/60 *1000) with_precision 2)) color: #indigo marker_size: (0.05) // size keyword instead of size in values
				marker_shape: marker_circle style: bar;
        ...
        display vidacity_lvl_1 type: opengl background: #lightgrey{
             ...
             overlay position: { 20, 490 } size: { 1200 #px, 250 #px } background: #black transparency: 0.3 border: #black rounded: true
            {
                float y <- 30#px;
                	...
                	draw ("TOTAL COMPLAINTS PER WEEK: " + total_complaints)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                	...
                	draw ("ACCUMULATED TIME OF BINS OVERFLOWED (MINS/WEEK): " + (total_time_bin_overflowed) with_precision 2)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                    ...
                    ask cleaner{                	
                    	draw ("CLEANER DISTANCE TRAVELLED (METRES/DAY): " + (self.total_time_travelled/60*self.speed*1000) with_precision 2)  at: { 285#px, y + 7#px } color: # white font: font("Helvetica", 25, #bold);
                    }
