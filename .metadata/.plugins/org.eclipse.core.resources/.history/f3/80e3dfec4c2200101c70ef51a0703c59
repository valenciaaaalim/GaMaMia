...
//	float ReadyToClear_multiplier <- 0.6 parameter:true category: "Experiment Settings";
	...
	int lvl1_store_1_no_tenants <- 3 parameter:true category:"Level 1";
...
species obstacle {
	aspect default {
		draw shape + (0.45/2.0) color: #gray border: #black depth: 4;
	}
}
...
species store{
	...
	init {	
		...
		write name + " contains " + length(members) + " tenants.";
...
//		ask people {
//			
//			if (self.store_host = myself) and (self.location distance_to self.store_host < 1) and !(myself.tenants_in_store contains self) {
//				myself.tenants_in_store <- 	myself.tenants_in_store + self;
//				
//			}
//			
//			else if (self.store_host = myself) and (self.location distance_to self.store_host > 1) and (myself.tenants_in_store contains self){
//				myself.tenants_in_store <- 	myself.tenants_in_store - self;
//			}
//		
//			
...
//	}
	...
	//    reflex generate_trash when: self.location = home_base and has_trash = false and every(5#cycle) {
	    reflex check_throw_trash	 when: self.location = home_base and has_trash = false {
...
//			draw ("Trash on hand:") color:#black size:displayTextSize at:{location.x-5,location.y+displayTextSize*2} perspective:false; 
			draw (string(round(trash_on_hand*100)/100)) color:#black size:displayTextSize at:{location.x+5,location.y+displayTextSize*4} perspective:false; 
	...
	aspect default {
		draw square(28) color: #burlywood at:{location.x,location.y, location.z-1};
		draw ("Store " + self.index + " has " + current_tenants_in_store + "/" + tenant_no + " tenants.") color:#black size:displayTextSize at:{location.x+5,location.y+5+displayTextSize} perspective:false;
	}
}
...
species cleaner skills:[pedestrian]{
	...
	// when cleaner is at rest or when cleaner has just finished a cleaning round
...
//	        	write "Order to clear the current 3 bins: " + bins_to_clean_order;
	        	bins_to_clean_order <- bins_to_clean_order sort_by (each.distance_to_central_bin);
	        	write "Order to clear the current 3 bins: " + bins_to_clean_order;
...
experiment pedestrian_navigation type: gui {
    ...
    output {
        display vidacity_lvl_1 type: opengl {
            ...
            species store aspect: default transparency: 0.48{
            	species people aspect: default transparency: 1;
            }
