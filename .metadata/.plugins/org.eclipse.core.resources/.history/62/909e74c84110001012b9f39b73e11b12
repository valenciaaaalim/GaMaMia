...
global {
    ...
    init {
        ...
        create Person number: nb_people/2 {
            ...
            floor <- 1;
            is_infected <- flip(0.1);  // 10% initially infected
            color <- (is_infected ? #pink : #lightgreen);
        }
        ...
        create Staircase{
        	location <- {(environment_width/2), (environment_height/2), floor_height};
        }
    }
    ...
    reflex update_statistics {
        ...
        int floor0_count <- Person count (each.floor = 0);
        write "Floor 0 has" + floor0_count + "people";
    }
}
...
species Person skills: [moving] {
    ...
    reflex search_target when: targets = nil{
    	ask targets at_distance(climb_range){
    		myself.targets <- list<Staircase>(self);
    	}
    }
    	
   	reflex change_level when: targets != nil{
    	ask Staircase at_distance(climb_range){
    		if (myself.floor = 0){
    			myself.floor <- 1;
    			myself.location <- myself.location + {0,0,floor_height};
    		}
    		else if (myself.floor = 1){
    			myself.floor <- 0;
    			myself.location <- myself.location - {0,0,floor_height};
    		}
    ...
    aspect default {
        ...
        if (is_infected and color = #green) {
        	color <- #red;
        	
        }
        else if (is_infected and color = #lightgreen) {
        	color <- #pink;
        	
        }
        else if (!is_infected and color = #red) {
        	color <- #green;
        	
        }
         else if (!is_infected and color = #pink) {
        	color <- #lightgreen;
...
species Staircase {
    ...
    aspect base {
        draw square(5) color: #blue depth:10 ;
